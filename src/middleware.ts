  ...
  if (isAdminSubdomain) {
    ...
    if (
      ...
      if (pathname === '/customers') {
        return NextResponse.rewrite(new URL('/admin/customers', req.url));
      }
      if (pathname.startsWith('/customers/')) {
        const id = pathname.replace('/customers/', '');
        return NextResponse.rewrite(new URL(`/admin/customers/${id}`, req.url));
      }
...
export const config = {
  matcher: [
     ...
     */
    '/((?!_next/static|_next/image|favicon|apple-touch-icon|android-chrome|icon|apple-icon|site\\.webmanifest|robots\\.txt|sitemap\\.xml|assets/|styles\\.css|script\\.js).*)',
  ],
};
